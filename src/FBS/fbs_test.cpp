...
int main(int argc, char * argv[])
{
    ...
    // upsampling dc;
    PARAM param;// upsampling parameter
    param.zoom = 0.5;
    param.sigma_chroma = 32;
    param.sigma_luma = 2.5;
    ...
    while(count <= best_frame) { // use the 5th result for best performance
        ...
        // dc.run(imgGray, imgFlood, dense);
        upsampling(imgFlood, imgGray, param, dense);
