...
int g_rangeSearch = 12;               //* search similar parts range 
int g_isimilarity_threshold = 995;  //* mimium similarity for moving fix
int g_iedge_threshold_guide = 15; //* threshold for edge of guide image
...
cv::Mat getSobelAbs_org(cv::InputArray src_, int tapsize_sobel, float minval = 0.0f)
{
...
cv::Mat getSobelAbs(cv::InputArray src_, int tapsize_sobel, float minval = 0.0f)
{
	...
	cv::Mat dst = h_sobels + v_sobels;
	return dst;
}

cv::Mat getSobelAbsII(cv::InputArray src_, int tapsize_sobel, float minval = 0.0f)
{
	const cv::Mat src = src_.getMat();
	std::vector<cv::Mat> t_sobels(2);
	cv::Mat h_sobels, v_sobels;
 ...
 * @param pcFlood 
 */
void filter_edge_by_depthAndGuide(cv::Mat& pcFlood, const cv::Mat& guide, float cx, float cy, float fx , float fy, 
                float featureMapDepth[80][60][8], float featureMapRGB[960][540][8])
{
    ...
    float threshold_edge_guide = (float)g_iedge_threshold_guide;
    ...
    cv::Mat imgEdge_o, imgEdge;
    imgEdge_o = getSobelAbs_org(matrixImg, 3);
    imgEdge = getSobelAbs(matrixImg, 3);
    cv::imshow("imgEdge_org", imgEdge_o);
    cv::imshow("imgEdge", imgEdge);
    // * extract edge of guide
    cv::Mat imgEdgeGuide_o, imgEdgeGuide, imgEdgeGuideMask, imgBoarder;
    imgEdgeGuide_o = getSobelAbs_org(guide, 3);
    imgEdgeGuide = getSobelAbs(guide, 3);



    imgEdgeGuideMask = cv::Mat::zeros(guide.size(), guide.type());
    imgEdgeGuideMask.setTo(1.0, imgEdgeGuide > threshold_edge_guide);
    cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(g_idilate_rgb, g_idilate_rgb));
    cv::dilate(imgEdgeGuideMask, imgBoarder, kernel);
    ...
    for (int r = 0; r < rows; ++r) { // ! loop the edge directly
        for (int c = 0; c < cols; ++c) {
            ...
            if (imgEdge.at<float>(r, c) >= threshold_edge || 
                imgBoarder.at<uchar>(v, u) != 0) { // fix candidates
                //
...
int main(int argc, char * argv[])
{
    ...
    const string fn_output = rootPath + "/00000000_upsampling_results_4.tiff";
    const string fn_outputFolder = "D:\\workspace2\\Upsampling\\DS5\\mis_upsampling\\dat\\real\\hand_test\\tuning_method4\\";
    ...
    cv::createTrackbar("edge_depth", "imshow", &g_iedge_threshold_depth, 120);
    cv::createTrackbar("edge_guide", "imshow", &g_iedge_threshold_guide, 100);
    cv::createTrackbar("edge dilate", "imshow", &g_idilate_rgb, 40);
    ...
    while(1) { // use the 5th result for best performance
        ...
        cout << "preprocessing time = " << duration << "[us]" << endl;
#endif
        extractDepthFeatures(pcFlood, featureMapDepth);
        // * filter the edge points
        filter_edge_by_depthAndGuide(pcFlood, imgGray, cx, cy, fx, fy, featureMapDepth, featureMapRGB);
    ...
    // cv::imshow("input match", markSparseImg); // * match input point with RGB
        ...
        cout << "FGS time = " << duration << "[ms]" << endl;
#endif
        t_end = std::chrono::system_clock::now();
        duration = std::chrono::duration_cast<std::chrono::milliseconds>(t_end - t_start).count();
        ...
        if (a == 's') {
            char szFn[255];
            sprintf(szFn, "%smethod3_%d_%d_%d_%d_%d_%d_%d.tiff", fn_outputFolder.c_str(), \
                    g_iedge_threshold_depth, g_iedge_threshold_guide, g_idilate_rgb, \
                    g_rgb_feature_span, g_rgb_half_block_size, \
